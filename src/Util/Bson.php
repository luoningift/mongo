...
use Kaikeba\Mongo\Types as Types;
    ...
    private static function encElement($name, $value)
    {
        switch (gettype($value)) {
            case 'object':
                ...
                } elseif ($value instanceof Types\MongoRegex) {
                    $bin = $value->regex . "\0" . $value->flags . "\0";
                    $sig  = self::ETYPE_REGEX;
                } elseif ($value instanceof Types\MongoDate) {
                    $ms = $value->getMs();
                    $bin = pack('V2', $ms & 0xffffffff, ($ms >> 32));
                    $sig = self::ETYPE_DATE;
                } elseif ($value instanceof Types\MongoTimestamp) {
                    $bin = pack('V2', $value->inc, $value->sec);
                    $sig = self::ETYPE_TIMESTAMP;
                } elseif ($value instanceof Types\MongoBinData) {
                ...
                } elseif ($value instanceof Types\MongoMaxKey) {
                    $bin = '';
                    $sig = self::ETYPE_MAXKEY;
                } elseif ($value instanceof Types\MongoMinKey) {
                    $bin = '';
    ...
    private static function decElement($data, &$offset)
    {
        ...
        switch ($sig) {
            case self::ETYPE_ID:
                ...
                $value = new Types\MongoId(str_pad(bin2hex($binId), 24, '0'));
                break;
            ...
            case self::ETYPE_CODE:
                ...
                $value = new Types\MongoCode($code, $scope);
                break;
            case self::ETYPE_REGEX:
                $regex = Util::parseCString($data, $offset);
                $flags = Util::parseCString($data, $offset);
                $value = new Types\MongoRegex('/' . $regex . '/' . $flags);
                break;
            case self::ETYPE_DATE:
                $vars = Util::unpack('V2i', $data, $offset, 8);
                $ms = ($vars['i2'] << 32) + $vars['i1'];
                $value = Types\MongoDate::createFromMs($ms);
                break;
            case self::ETYPE_TIMESTAMP:
                $vars = Util::unpack('V2i', $data, $offset, 8);
                $value = new Types\MongoTimestamp($vars['i2'], $vars['i1']);
                break;
            case self::ETYPE_BINARY:
                ...
                $value = new Types\MongoBinData($bin, $subtype);
                $offset += strlen($bin);
                break;
            case self::ETYPE_MAXKEY:
                $value = new Types\MongoMaxKey;
                break;
            case self::ETYPE_MINKEY:
                $value = new Types\MongoMinKey;
                break;
            default:
